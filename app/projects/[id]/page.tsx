    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      <header className="fixed top-4 left-1/2 -translate-x-1/2 z-50">
        <nav className="flex items-center gap-1 px-2 py-2 bg-[#0a0a0a]/70 backdrop-blur-xl border border-[#262626] rounded-full shadow-2xl">
          <Link href="/" className="flex items-center gap-2 px-4 py-2 text-gray-400 hover:text-white rounded-full hover:bg-[#262626] transition-all duration-200 font-medium">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            <span className="hidden lg:inline">Home</span>
          </Link>
          <a href="/#projects" className="flex items-center gap-2 px-4 py-2 text-gray-400 hover:text-white rounded-full hover:bg-[#262626] transition-all duration-200 font-medium">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
            <span className="hidden lg:inline">Projek</span>
          </a>
          <a href="/#services" className="flex items-center gap-2 px-4 py-2 text-gray-400 hover:text-white rounded-full hover:bg-[#262626] transition-all duration-200 font-medium">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
            </svg>
            <span className="hidden lg:inline">Layanan</span>
          </a>
          <a href="/#contact" className="flex items-center gap-2 px-4 py-2 text-gray-400 hover:text-white rounded-full hover:bg-[#262626] transition-all duration-200 font-medium">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <main className="pt-24 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
        <Link href="/#projects" className="inline-flex items-center gap-2 text-gray-400 hover:text-[#c41e2e] transition-colors mb-6 group">
          <div className="w-8 h-8 rounded-full bg-[#262626] group-hover:bg-[#c41e2e]/20 flex items-center justify-center transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-[#171717] rounded-3xl shadow-xl overflow-hidden border border-[#262626]">
          <div className="relative h-72 md:h-[450px] bg-gradient-to-br from-[#262626] to-[#171717]">
            <Image src={project.image} alt={project.title} fill className="object-cover" priority />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
            <div className="absolute top-4 right-4 flex gap-2">
              <span className="px-4 py-2 bg-[#0a0a0a]/60 backdrop-blur-md text-white font-semibold rounded-full text-sm">{project.year}</span>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8">
              <div className="md:col-span-2">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl font-bold text-white">Deskripsi Projek</h2>
                </div>
              ...
              <div className="md:col-span-1">
                <div className="bg-[#262626]/50 rounded-2xl p-6 border border-[#262626]">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="font-bold text-white">Teknologi</h3>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {project.technologies.map((tech, index) => (
                      <span key={index} className="px-3 py-1.5 bg-[#171717] border border-[#262626] text-gray-300 text-sm font-medium rounded-lg hover:border-[#c41e2e] hover:text-[#c41e2e] transition-colors cursor-default">{tech}</span>
                    ))}
                  </div>
                </div>
        ...
        <div className="mt-16">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="text-2xl font-bold text-white">Projek Lainnya</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {projects.filter((p) => p.id !== project.id).slice(0, 3).map((relatedProject) => (
              <Link key={relatedProject.id} href={`/projects/${relatedProject.id}`} className="group bg-[#171717] rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-[#262626] hover:border-[#c41e2e]/50">
                <div className="relative h-44 bg-gradient-to-br from-[#262626] to-[#171717] overflow-hidden">
                  ...
                  <div className="absolute top-3 right-3">
                    <span className="px-2 py-1 bg-[#0a0a0a]/80 backdrop-blur-sm text-white text-xs font-medium rounded-md">{relatedProject.year}</span>
                  </div>
                </div>
                <div className="p-5">
                  <h3 className="font-bold text-white mb-2 group-hover:text-[#c41e2e] transition-colors line-clamp-2">{relatedProject.title}</h3>
                  <div className="flex flex-wrap gap-1">
                    {relatedProject.technologies.slice(0, 2).map((tech, index) => (
                      <span key={index} className="px-2 py-0.5 bg-[#262626] text-gray-400 text-xs rounded">{tech}</span>
                    ))}
                  </div>
                </div>
              </Link>
